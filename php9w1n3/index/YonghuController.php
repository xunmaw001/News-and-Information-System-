...
class YonghuController extends CommonController {
    ...
    public function lists(){
        ...
        if ($base['isAdmin']!=1){
            ...
            if (isset($base['success'][$colum]) && in_array($colum, $columData)){
                $where .= " and `yonghuming` = '".$base['success'][$colum]."'";
            }
        }
                                                                                                                        		                                        $sql = "select * from `yonghu`".$where;
        $count = table_sql($sql);
    ...
    public function remind($columnName,$type){
        ...
        if ($type == 1){//数字
            $sql = "select * from `yonghu` where ".$columnName."<='".$remindEnd."' and ".$columnName.">='".$remindStart."'";
        }else{
            $sql = "select * from `yonghu` where ".$columnName."<='".date("Y-m-d",strtotime("+".$remindStart." day"))."' and ".$columnName.">='".date("Y-m-d",strtotime("+".$remindStart." day"))."'";
        }
        $result = table_sql($sql);
