...
class DiscussxinwenxinxiController extends CommonController {
    ...
    public function remind($columnName,$type){
        ...
        if ($type == 1){//数字
            $sql = "select * from `discussxinwenxinxi` where ".$columnName."<='".$remindEnd."' and ".$columnName.">='".$remindStart."'";
        }else{
            $sql = "select * from `discussxinwenxinxi` where ".$columnName."<='".date("Y-m-d",strtotime("+".$remindStart." day"))."' and ".$columnName.">='".date("Y-m-d",strtotime("+".$remindStart." day"))."'";
        }
        $result = table_sql($sql);
